...
//       console.log(
//         `Created product selection job: ${productSelectionJob._id} for project: ${project._id}`,
//       );
//     }
...
//       "status currentStep mode quickModeProducts results selectedCompanies patentData",
//     );
...
//     const SUBSTEP_WEIGHT = {
//       infringementStoryline: 0.2,
//       claimChartA: 0.4,
//       webQueries: 0.6,
//       exaSearch: 0.8,
//       productFinalChart: 0.95,
//     };
...
//         return (
//           progress +
//           perProductWeight * (SUBSTEP_WEIGHT[job.currentSubStep] || 0.1)
//         );
...
//         if (j.currentSubStep)
//           currentStepName = `${j.productName}: ${j.currentSubStep}`;
//       });

//       const pcJob = jobOfType("projectCompletion");
//       if (pcJob?.status === "processing") {
//         progress = Math.max(progress, 97);
//         currentStepName = "Finalising report";
//       }
//       if (pcJob?.status === "completed") {
//         progress = 100;
...
//       if (psJob?.status === "processing" || psJob?.status === "pending") {
//         progress += 5;
//         currentStepName = "Selecting products";
//       }
//       if (psJob?.status === "completed") {
//         progress += INTERACTIVE_PS_WEIGHT;
//         currentStepName = "Products selected";
//       }
...
//       const productJobs = jobsOfType("productAnalysis");
//       productJobs.forEach((j) => {
//         progress = addSubStepProgress(j, perProdWeight, progress);
//       });
//       const pcJob = jobOfType("projectCompletion");
//       if (pcJob?.status === "processing") {
//         progress = Math.max(progress, 97);
...
//       .limit(limit)
//       .select(
//         "email patentId patentData.biblioData status createdAt completedAt mode currentStep",
//       );
