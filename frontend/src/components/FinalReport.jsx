...
import { useGetProjectDetailsQuery } from "../features/patents/patentApiSlice";
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto p-4">
      ...
      <div className="bg-white p-5 rounded-xl border flex justify-between">
        <h1>{project?.patentId}</h1>
        <p>{project?.results?.productAnalysis?.results?.[0]?.company}</p>
      </div>

      {/* 2. Claim Chart Rows - Map over backend results */}
      <div className="bg-white border rounded-xl overflow-hidden mt-6">
        {project?.results?.finalClaimChart?.[0]?.claimChart?.map((row, idx) => (
          <div key={idx} className="grid grid-cols-12 p-6 border-b">
            <div className="col-span-4 font-bold">{row.claimText}</div>
            <div className="col-span-8">{row.analysis}</div>
          </div>
        ))}
      </div>
    </div>
