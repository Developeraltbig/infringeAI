...
import { useStartQuickAnalysisMutation } from "../features/api/patentApiSlice";
...
const SearchArea = memo(() => {
  ...
  const [startAnalysis] = useStartQuickAnalysisMutation();

  const handleExecute = useCallback(async () => {
    const id = inputValue.trim();
    if (!id) return toast.warning("Please enter a patent ID");

    setShowModal(true); // 1. Show the static pop-up immediately

    try {
      // 2. Call the backend
      const response = await startAnalysis(id).unwrap();

      // 3. On success, redirect to result page using the ID from backend
      // We add a tiny timeout so the user actually sees the modal for a split second
      setTimeout(() => {
        setShowModal(false);
      ...
      setShowModal(false);
      toast.error(err?.data?.error || "Connection Error");
    }
  }, [inputValue, startAnalysis, navigate]);
  ...
  return (
    <div className="w-full max-w-3xl mx-auto">
      ...
      <div className="w-full bg-white rounded-2xl shadow-sm border p-2 flex flex-col">
        <div className="flex items-center w-full relative">
          <input
            type="text"
            value={inputValue}
            ...
            className="w-full px-6 py-4 text-lg outline-none bg-transparent"
          />
          <button
            onClick={handleExecute}
            className="absolute right-4 bg-[#ff6b00] hover:bg-orange-600 text-white p-3 rounded-full transition-all"
          >
            <Send size={20} />
          </button>
        </div>
